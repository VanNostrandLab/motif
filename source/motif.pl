...
use warnings;
use strict;
# source activate evn_homer
#20210608 - changed background to also use the 'extend peaks by N bases' code

my $usage = "Identify motifs for eCLIP peak clusters.

Usage:
motif.pl annotated_peak_bed [species] [outdir] [uid] [l10p] [l2fc] [processes]
...
my ($bed, $species, $output_dir, $uid, $l10p_cutoff, $l2fc_cutoff, $processes) = @ARGV;
if (not defined $bed) {
    die "$usage\n";
}
die "Error - The specified annotated bed file $bed does not exist!." unless -e $bed;

if (not defined $species) {
    $species = "hg19";
}
unless ($species eq "hg19") {
    die "Error - not really tested for non hg19\n";
}
...
if (not defined $l10p_cutoff) {
    $l10p_cutoff = 3;
}

if (not defined $l2fc_cutoff) {
    $l2fc_cutoff = 3;
}

if (not defined $processes) {
    $processes = 1;
}
...
my $five_pad = 20;
my $three_pad = 5;
my %acceptable_chroms;
for my $n (1..22,"X","Y","M") {
    $acceptable_chroms{"chr".$n} = 1;
}

my $working_output_dir = $output_dir.$uid.".motifs.40.min.plus.5p.".$five_pad.".3p.".$three_pad."/";
...
open(BED,$bed) || die "Failed to open $bed for reading!\n";
for my $line (<BED>) {
    chomp($line);
    my @tmp = split(/\t/,$line);
    my $chr = $tmp[0];
...
## extend peaks N bp in the 5' and 3' direction
    if ($str eq "+") {
	$start -= $five_pad;
	$stop += $three_pad;
    } elsif ($str eq "-") {
...
#    for (my $i=0;$i<$n_bgd_per_event;$i++) {
	...
	next unless (exists $acceptable_chroms{$featurechr});

	my ($featurestart,$featurestop) = split(/\-/,$featurepos);

	my $bgdpeak_size = $featurestop - $featurestart;
	if ($bgdpeak_size >= 40) {
	} else {
	    my $bgddiff = 40 - $bgdpeak_size;

	    if ($bgddiff % 2 eq "0") {
		$featurestart -= int($bgddiff / 2);
		$featurestop += int($bgddiff / 2);
	    } else {
		if ($featurestr eq "+") {
		    $featurestart -= (int($bgddiff / 2)+1);
		    $featurestop += int($bgddiff / 2);
		} elsif ($featurestr eq "-") {
		    $featurestart -= int($bgddiff / 2);
		    $featurestop += int($bgddiff / 2)+1;
		}
	    }
	}
## extend peaks 10 bp in the 5' direction                                                                                                                
	if ($featurestr eq "+") {
	    $featurestart -= $five_pad;
	    $featurestop += $three_pad;
	} elsif ($featurestr eq "-") {
	    $featurestop += $five_pad;
	    $featurestart -= $three_pad;
	}
	...
	my $start_offset = int($final_peak_size / 2);
	my $stop_offset = $final_peak_size - $start_offset;
...
for my $annot ("all",keys %annotation_hash) {
    ...
    my $cmd = "findMotifs.pl $outexptfi fasta $homer_out_dir -nofacts -p $processes -rna -S 20 -len 5,6,7,8,9 -noconvert -nogo -fastaBg $outbgdfi > /dev/null 2>&1";
    print STDERR "$cmd\n";
    system($cmd);

    my $homer_out = $homer_out_dir."homerMotifs.all.motifs";
    $cmd = "compareMotifs.pl $homer_out $homer_out_dir > /dev/null 2>&1";
    print STDERR "$cmd\n";
    system($cmd);

    my $homer_html = $homer_out_dir."homerResults.html";
    my $new_homer_html = $working_output_dir.$uid.".".$annot.".homer.results.html";
    system("cp $homer_html $new_homer_html");
    system("mv $outexptfi $homer_out_dir.$uid.$annot.expect.fa");
    system("mv $outbgdfi $working_output_dir.$uid.$annot.background.fa");
}
